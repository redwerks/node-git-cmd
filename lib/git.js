		...
		return new Promise(function(resolve, reject) {
			...
			if ( runOpts.output === 'capture' ) {
				...
				if ( _captureArray ) {
					...
					_capturePromise = Promise.cast(getRawBody(stream, {encoding: runOpts.encoding}));
				}
			...
			return run(opts)
				.then(function(c) {
					if ( c.code === 0 ) {
						...
						throw new Error(opts.prefix + fmt('git returned exit code %d', c.code));
					}
				});
		},
